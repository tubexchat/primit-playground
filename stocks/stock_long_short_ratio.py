...
# -*- coding: utf-8 -*-
"""
股票多空比获取脚本 (Stock Long/Short Ratio)
===========================================

调用 Primit 数据中心的两个多空比接口（同一份 schema，按 --type 选择）：
    GET https://api.pipai.org/stock/v1/whaleLongShortRatio   鲸鱼（大户）多空比
    GET https://api.pipai.org/stock/v1/topLongShortRatio     头部账户多空比

请求参数
--------
    symbol   string    是   —     股票代码，例如 NVDA、AAPL。
...
----
    时间序列数组，每个元素：
        {"symbol": "NVDAUSDT", "longAccount": "0.7193", "shortAccount": "0.2807",
         "longShortRatio": "2.5624", "timestamp": 1782360000000}
    longShortRatio = longAccount / shortAccount；timestamp 为 Unix 毫秒。

用法示例
--------
    python stocks/stock_long_short_ratio.py                         # 默认 whale, NVDA, 4h
    python stocks/stock_long_short_ratio.py --type top --symbol AAPL
    python stocks/stock_long_short_ratio.py --period 1d --limit 60
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="获取 Primit 股票多空比 (whale / top long-short ratio)。",
    ...
    parser.add_argument("--symbol", default="NVDA", help="股票代码，例如 NVDA、AAPL")
